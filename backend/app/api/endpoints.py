...
async def create_return_request(payload: ReturnCreateRequest):
    ...
    # 3. Persistence inside returns collection
    ...
    # Generate labels (generates PDF and stores in GridFS)
    ...
    if db.returns is not None:
        try:
            ...
            await db.returns.insert_one(return_request_doc.copy())
        ...
        try:
            ...
            if ret:
                items = [
                    {
                        ...
                        "exchange_variant": i.get("exchange_variant")
                    }
                ...
                return ReturnDetailResponse(
                    ...
                    order_number=ret["order_details"]["order_number"],
                    email=ret["customer_details"]["email"],
                    method=ret["return_method"],
                    items=items,
                    ...
                    shipping_label_url=shipping.get("shipping_label_url"),
...
async def download_shipping_label_pdf(return_id: str):
    """Serves return shipping label PDF document from GridFS."""
    if db.returns is not None:
        try:
            ret = await db.returns.find_one({"return_id": return_id})
            if ret and "shipping" in ret and "label_file_id" in ret["shipping"]:
                label_file_id = ret["shipping"]["label_file_id"]
                file_stream, filename, content_type = await GridFSService.get_file(label_file_id)
                
                async def file_generator():
                    while chunk := file_stream.read(1024 * 64):
                        yield chunk
                        
                return StreamingResponse(file_generator(), media_type=content_type, headers={
                    "Content-Disposition": f"attachment; filename={filename}"
                })
        except Exception as e:
            logger.error(f"Failed to fetch and download shipping label PDF: {e}")
            
    raise HTTPException(status_code=404, detail="Shipping label PDF not found.")
